...
let carScale = 1;
let targetScale = 1.5;     // 最終放大到 1.5 倍
let scaling = false;
let scalingSpeed = 0.01;   // 放大速度（可調整）
...
function touchStarted() {
  ...
  return false; // 阻止觸發 mousePressed 避免重複
}

function handlePress(px, py) {
  let scaleFactor = canvasSize / baseWidth;
  let scaledX = px / scaleFactor;
  let scaledY = py / scaleFactor;

  let d = dist(scaledX, scaledY, keyX, keyY);
  /*if (d < 30) {
    engineOn = !engineOn; // 切換引擎狀態
  }*/
  if (d < 30) {
    engineOn = !engineOn;
    if (engineOn) {
      scaling = true;
      carScale = 1; // 從原始大小開始放大
    } else {
      carScale = 1; // 引擎關閉時縮回原始大小
      scaling = false;
    }
  }
}
...
function drawF1Text() {
  ...
  // 等速放大動畫
  if (scaling && carScale < targetScale) {
    carScale += scalingSpeed;
    if (carScale >= targetScale) {
      carScale = targetScale;
      scaling = false; // 放大完成後停止
    }
  }

  scale(carScale); // 加入放大比例
